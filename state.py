...
PLUGIN_STATE_KEYS = {
    ...
    "previous_tts_provider",
    "auto_prompt_constraint",
}

PROMPT_START = "<<<MINIMAX_TTS_TONE_CONSTRAINT_START>>>"
PROMPT_END = "<<<MINIMAX_TTS_TONE_CONSTRAINT_END>>>"
_LEGACY_PROMPT_CONSTRAINT_UNUSED = f"""{PROMPT_START}
MiniMax TTS 翻译语气约束：
- 当模板要求输出 translate 字段时，translate 字段会作为 MiniMax 文生音输入；生成日语译文时，可以在对应情绪位置少量加入 MiniMax 支持的语气标签，例如 (laughs)、(sighs)、(breath)、(gasps)、(crying)。
- 语气标签只放在 translate 字段，不要放进 speech 字段；speech 仍保持自然简体中文。
- 标签要服务于原台词情绪，不要连续堆叠，不要每句都加；平静说明、技术结论和风险提醒可以不加。
- 姓名“海漫華淡/華淡”的读音固定为 Katan / かたん，不要翻成 Hanatan、Hanadan 或花淡。
{PROMPT_END}"""

PROMPT_CONSTRAINT = f"""{PROMPT_START}
当模板要求输出 translate 字段时，translate 字段会作为 MiniMax 文生音输入。因此，日语译文可以根据台词情绪，在合适位置少量加入 MiniMax 支持的语气标签，用于辅助语音表现。
...
标签要服务于当前台词情绪，不要为了装饰而乱加。
只有在情绪变化明显时，才允许同一句中使用 2 个标签，但不要连续堆叠。
标签应放在最能表现情绪的位置。
通常可以放在句首，例如：
...
def set_minimax_extra(extra: dict[str, Any], *, take_over: bool = False) -> None:
    data = load_api_config()
    all_extra = data.get("tts_extra_configs")
    if not isinstance(all_extra, dict):
        all_extra = {}
...
def restore_previous_tts_provider(previous: str | None = None) -> None:
    target = str(previous or "").strip().lower()
    if not target or target == PROVIDER_SLUG:
        target = "gpt-sovits"
    data = load_api_config()
    if str(data.get("tts_provider") or "").strip().lower() == PROVIDER_SLUG:
        data["tts_provider"] = target
        save_api_config(data)
...
def remove_prompt_constraint_text(text: str) -> str:
    src = text or ""
    pattern = re.compile(
        rf"\n*\s*{re.escape(PROMPT_START)}.*?{re.escape(PROMPT_END)}\s*\n*",
        re.DOTALL,
    )
    return pattern.sub("\n", src).strip()
...
def prompt_constraint_active() -> bool:
    """只有插件启用且 MiniMax 已接管主 TTS 时，才向模板注入语气约束。"""
