...
本文档整理 Cloud TTS 插件当前内置的默认提示词约束，来源于 `state.py` 中的 `build_default_constraint_text()` 逻辑。当前插件版本为 `0.9.3`。

从 `0.9.0` 开始，「默认模板」和每个角色都会拥有中文、日语、粤语、英语四套默认提示词版本。从 `0.9.3` 开始，配置页里的模板语言下拉框只用于编辑对应模板；运行时实际注入哪一种语言，由主程序当前语音语言决定。
