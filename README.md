...
- voice_id 查找顺序为：角色绑定 voice_id、默认兜底 voice_id、已缓存的克隆 voice_id、可选的本地参考音频自动克隆。
...
- 选择角色后，在角色语音配置里为该角色选择 `本地参考音频`，再点击 `上传本地参考音频并生成 voice_id`，用该本地音频创建 MiniMax 克隆声线。
