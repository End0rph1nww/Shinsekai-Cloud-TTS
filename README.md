...
MiniMax speech-2.x 语音合成插件。它会向 Shinsekai 注册一个新的 TTS 引擎 `minimax-tts`，并提供独立的插件设置页，用于配置模型、声线克隆、voice_id、提示词约束和合成参数。
...
- 提示词模板支持中文、日语、粤语、英语四套默认母版；每个角色也会自动生成四套默认提示词，可在插件设置页按角色选择、修改和保存。
...
- 克隆时是否启用降噪、音量归一。
- 自动克隆缓存路径，默认是 `cache/audio/cloud_tts_voice_cache.json`。
...
- 如主程序保存的角色参考音频被裁短，可在角色语音配置里为该角色选择 `本地参考音频`。手动上传和运行时自动克隆都会优先使用这个本地音频文件。

上传参考音频前，必须先在主菜单 API 设置页保存 `MiniMax API KEY`。如果没有配置本地参考音频，插件会从 `characters.yaml` 中读取角色的 `refer_audio_path` 作为参考音频路径。
